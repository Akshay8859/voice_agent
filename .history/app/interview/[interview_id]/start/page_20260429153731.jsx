            ...
            <div className="w-[340px] flex flex-col gap-4">
              <div className="bg-gray-50 rounded-xl p-5 shadow flex flex-col gap-2">
                ...
                <div className="mt-2 text-gray-700 font-medium">
                  {interviewInfo?.interviewContent?.questionList?.map((item, index) => (
                    <div>
                      <span className="font-semibold text-green-700">Question {index + 1} of {interviewInfo?.interviewContent?.questionList.length}</span>
                      <p key={index} className={`mb-2 ${index === 0 ? 'text-blue-600' : 'text-gray-700'}`}>
                        {item.question}
                      </p>
                    </div>
                  ))}
