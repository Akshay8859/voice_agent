...
const StartInterview = () => {
    ...
    const GenerateFeedback = async () => {
        ...
        const {data, error} = await supabase
            ...
            .insert([
                { userName: interviewInfo?.userName, userEmail: interviewInfo?.userEmail, interviewId: interview_id, feedbackData: parsed, recommended: false }
            ])
            .select();
    ...
    <div className='p-20 lg:px-48 xl:px-56'>
        ...
        <div className='flex items-center justify-center gap-5 mt-7'>
            ...
            <AlertConfirmation stopInterview={() => stopInterview()}>
                {!loading ? <Phone className='h-12 w-12 p-3 bg-red-500 text-white rounded-full cursor-pointer' onClick={() => stopInter} /> : <Loader2Icon className='animate-spin' />}
            </AlertConfirmation>
        </div>
